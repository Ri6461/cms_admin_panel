...
def read_metadata_items(skip: int = 0, limit: int = 10, db: Session = Depends(get_db), current_user: models.User = Depends(get_current_active_user_with_role("Admin", "Super Admin"))):
    check_permissions(current_user, "metadata", "read")
...
def create_metadata_item(metadata_item: schemas.MetaDataItemCreate, db: Session = Depends(get_db), current_user: models.User = Depends(get_current_active_user_with_role("Admin", "Super Admin"))):
    check_permissions(current_user, "metadata", "create")
...
def update_metadata_item(metadata_item_id: int, metadata_item_update: schemas.MetaDataItemUpdate, db: Session = Depends(get_db), current_user: models.User = Depends(get_current_active_user_with_role("Admin", "Super Admin"))):
    check_permissions(current_user, "metadata", "update")
...
def delete_metadata_item(metadata_item_id: int, db: Session = Depends(get_db), current_user: models.User = Depends(get_current_active_user_with_role("Admin", "Super Admin"))):
    check_permissions(current_user, "metadata", "delete")
